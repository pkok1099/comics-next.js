...
async function scrapeComicInfo(komik) {
  ...
  try {
    ...
    data.chapterAwal = $('.chapter-awal a').attr(
      'href',
    ); // Link Chapter Awal
    data.chapterBaru = $(
      '.epsbaru .epsbr:not(.chapter-awal) a',
    ).attr('href'); // Link Chapter Baru
    data.rating = $('i[itemprop="ratingValue"]')
      .text()
      .trim(); // Rating
    ...
    data.lastUpdated =
      $('.shortcsc.sht2 p')
        .text()
        .match(
          /Update chapter terbaru komik.*?adalah tanggal (.*?)/,
        )?.[1] || 'Last updated date not found';
    ...
    // Spoiler images
    data.spoilerImages = $(
      '.spoiler .spoiler-img img',
    )
      .map((_, img) => $(img).attr('src'))
      .get()
      .filter(Boolean) || [
      'No spoiler images found',
    ];
