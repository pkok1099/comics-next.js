...
const KomikList = () => {
  ...
  const debouncedFetchKomik = useMemo(() => debounce(fetchKomik, 500), [fetchKomik]);
  ...
  const handleKomikClick = (komikLink) => {
    const komikSlug = komikLink.replace(/https:\/\/[^]+\/manga\/([^]+)\//, '$1');
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-800 text-white p-5">
      <form onSubmit={handleSearchSubmit} className="w-full max-w-lg mb-5">
        <input
          type="text"
