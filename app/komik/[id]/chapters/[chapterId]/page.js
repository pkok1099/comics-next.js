...
const fetchChapterImages = useCallback(async (chapterId) => {
  try {
    ...
    const response = await fetch(
      `${apiUrl}/api/komik/${decodeURIComponent(id)}/chapter/${chapterId}/images`
    );
  ...
  const loadChapterImages = useCallback(async (chapterId) => {
  setLoading(true);
...
}, [fetchChapterImages]); // Menambahkan fetchChapterImages sebagai dependensi
  ...
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "";
    const response = await fetch(
      `${apiUrl}/api/komik/${decodeURIComponent(id)}/chapters`
    );
  ...
  const goToNextChapter = () => {
    router.push(
      `/komik/${decodeURIComponent(id)}/chapters/${parseInt(chapterId) + 1}`
    );
  };
