...
const ChapterDetail = () => {
  ...
  const fetchChapterImages = useCallback(
    async (chapterId) => {
      try {
        ...
        const response = await fetch(`${apiUrl}/api/komik/${decodeURIComponent(id)}/chapter/${chapterId}/images`);
  ...
  const fetchChapterList = useCallback(async () => {
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || '';
      const response = await fetch(`${apiUrl}/api/komik/${decodeURIComponent(id)}/chapters`);
  ...
  const goToNextChapter = () => {
    router.push(`/komik/${decodeURIComponent(id)}/chapters/${parseInt(chapterId) + 1}`);
  };
    ...
    <div className="chapter-container relative">
      ...
      {buttonVisible && (
        <div className="fixed right-2 sm:right-4 md:right-8 top-1/2 transform -translate-y-1/2 flex flex-col gap-4 z-50">
          <button
            onClick={() => router.push(`/komik/${decodeURIComponent(id)}/chapters/${parseInt(chapterId) - 1}`)}
          ...
          <button
            ...
            className={`opacity-75 h-10 sm:h-12 md:h-14 rounded-lg bg-gray-700 text-white flex items-center justify-center shadow-lg transform transition-transform duration-500 hover:rotate-360 hover:opacity-80 ${
              isFetching ? 'hidden' : ''
            }`}
          >
            <span className="[writing-mode:vertical-rl] rotate-360 text-xs sm:text-sm md:text-base lg:text-lg">
