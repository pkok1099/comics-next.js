...
(async () => {
  try {
    ...
    const synopsis = $(".pb-2 strong:contains('Sinopsis:')").next().text().trim();
    const genre = $('.tags a')
    ...
    const created = $("td:contains('Created Date')").next().text().trim();
    const chapters = $('#chapter_list ul li')
      .map((_, el) => {
        ...
        return { chapterTitle, chapterLink, chapterDate };
      })
      .get();
