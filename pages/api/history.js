...
import { connectToDatabase, insertHistory, updateHistory, findHistory } from '@/utils/mongodb';
...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      const existingHistory = await findHistory(collection, user, title);

      if (existingHistory) {
        // Jika data chapter sebelumnya ada, update chapterId dan timestamp-nya
        await updateHistory(collection, existingHistory._id, chapterId);
      ...
      await insertHistory(collection, user, title, chapterId);

      res.status(200).json({ message: 'History saved successfully' });
    } catch (err) {
      res.status(500).json({ message: 'Error saving or updating history', error: err.message });
    } finally {
      await client.close();
    }
  } else {
