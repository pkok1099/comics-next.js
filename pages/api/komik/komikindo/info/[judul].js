...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const chapters = await komikindo.scrapeComicInfo(judul);

      // Jika chapters tidak ditemukan
      if (!chapters || chapters.length === 0) {
        return res.status(404).json({ message: 'Chapter list not found' });
      }
      ...
      return res.status(500).json({ message: 'Error retrieving chapter list' });
    }
  } else {
