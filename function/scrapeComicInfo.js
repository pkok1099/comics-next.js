...
async function scrapeComicInfo(komik) {
  ...
  try {
    ...
    // Status
    data.status =
      $(".spe span").eq(1).text().replace("Status:", "").trim() ||
      "Status not found";

    // Author and Illustrator
    data.author =
      $(".spe span").eq(2).text().replace("Pengarang:", "").trim() ||
      "Author not found";
    data.illustrator =
    ...
    // Comic type
    data.type =
      $(".spe span").eq(6).text().replace("Jenis Komik:", "").trim() ||
      "Comic type not found";

    // Official source
    data.officialSource =
      $(".spe span").eq(7).find("a").attr("href") || "Not available";
